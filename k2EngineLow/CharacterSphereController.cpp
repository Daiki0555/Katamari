...
namespace nsK2EngineLow {
	...
	void CharacterSphereController::Init(float radius, const Vector3& position)
	{
		m_position = position;
		//コリジョン作成。
		m_radius = radius;
	...
	const Vector3& CharacterSphereController::Execute(Vector3& moveSpeed, float deltaTime)
	{
		if (moveSpeed.y > 0.0f) {
			//吹っ飛び中にする。
			m_isJump = true;
	...
	* @brief	死亡したことを通知。
	*/
	void CharacterSphereController::RemoveRigidBoby()
	{
